...
def log_device_used(output_file="output.log"):
    """
    Logs whether GPU or CPU is being used to the specified output file.
    """
...
camels_data_tensor = torch.tensor(camels_data, dtype=torch.float32).unsqueeze(1)  # Shape: [ 1, 256, 256]
camels_data_resized = F.interpolate(camels_data_tensor, size=(64, 64), mode='bilinear') # Shape: [15000, 1, 64, 64]
...
selected_images = camels_data_tensor[random_indices]  # Shape: [10, 1, 256, 256]
selected_images_resized = F.interpolate(selected_images, size=(64, 64), mode='bilinear')  # Shape: [10, 1, 64, 64]
...
# Save the means to a file in the output directory
...
# Save a histogram comparing the means
