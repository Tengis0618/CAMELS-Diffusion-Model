...
output_dir = f"outputs/paper_lr_{lrate}_epochs_{n_epoch}_timesteps_{timesteps}_params_{num_params}"
...
val_loss_log = []
likelihood_log = []
val_likelihood_log = []  # Added validation likelihood log
elbo_log = []
bpd_log = []
val_elbo_log = []
val_bpd_log = []
...
for ep in range(n_epoch):
    ...
    if ep % 5 == 0 or ep == n_epoch - 1:
        ...
        # Calculate likelihood on a subset of training set
        subset_size = min(len(train_dataset), 200)
        ...
        val_subset_size = min(len(test_dataset), 200)
        val_subset_indices = random.sample(range(len(test_dataset)), val_subset_size)
        val_subset_dataset = torch.utils.data.Subset(test_dataset, val_subset_indices)
        val_subset_dataloader = DataLoader(val_subset_dataset, batch_size=batch_size, shuffle=False)
        
        val_neg_log_likelihood = calculate_likelihood(
            nn_model, val_subset_dataloader, timesteps, device, ab_t, b_t, a_t
        )
        val_likelihood_log.append(val_neg_log_likelihood)
        ...
        with open(timing_log_path, "a") as f:
            ...
            f.write(f"  Train Negative Log Likelihood: {neg_log_likelihood:.6f}\n")
            f.write(f"  Val Negative Log Likelihood: {val_neg_log_likelihood:.6f}\n")
        ...
        print(f"Train NLL: {neg_log_likelihood:.6f}, Val NLL: {val_neg_log_likelihood:.6f}")
...
with open(timing_log_path, "a") as f:
    ...
    f.write(f"Final training negative log likelihood: {likelihood_log[-1]:.6f}\n")
    f.write(f"Final validation negative log likelihood: {val_likelihood_log[-1]:.6f}\n\n")
...
plt.subplot(2, 2, 1)
plt.plot(range(1, n_epoch+1), np.log(loss_log), color='orange', label='Training', linewidth=2)
plt.plot(eval_x, np.log(val_loss_log), 'o-', color='blue', label='Validation', linewidth=2, markersize=6)
plt.xlabel("Epoch", fontsize=25)
plt.ylabel("Log Loss", fontsize=25)
plt.legend(fontsize=25)
plt.grid(True, alpha=0.7)
plt.title(f"Training Metrics", fontsize=28)

# Plot 2: Negative log likelihood (both training and validation)
plt.subplot(2, 2, 2)
plt.plot(eval_x, likelihood_log, 'o-', color='orange', label='Training', linewidth=2, markersize=6)
plt.plot(eval_x, val_likelihood_log, 'o-', color='blue', label='Validation', linewidth=2, markersize=6)
plt.xlabel("Epoch", fontsize=25)
plt.ylabel("NLL", fontsize=25)
plt.legend(fontsize=25)
plt.grid(True, alpha=0.7)
plt.title("Negative Log Likelihood Evolution", fontsize=28)

# Plot 3: ELBO
plt.subplot(2, 2, 3)
plt.plot(eval_x, elbo_log, color='orange', label='Training', linewidth=2)
plt.plot(eval_x, val_elbo_log, 'o-', color='blue', label='Validation', linewidth=2, markersize=6)
plt.xlabel("Epoch", fontsize=25)
plt.ylabel("ELBO", fontsize=25)
plt.legend(fontsize=25)
plt.grid(True, alpha=0.7)
plt.title("ELBO Evolution", fontsize=28)

# Plot 4: BPD
plt.subplot(2, 2, 4)
plt.plot(eval_x, bpd_log, color='orange', label='Training', linewidth=2)
plt.plot(eval_x, val_bpd_log, 'o-', color='blue', label='Validation', linewidth=2, markersize=6)
plt.xlabel("Epoch", fontsize=25)
plt.ylabel("Bits Per Dimension (BPD)", fontsize=25)
plt.legend(fontsize=25)
plt.grid(True, alpha=0.7)
plt.title("BPD Evolution", fontsize=28)
...
def visualize_reconstruction_comparison(original_images, reconstructed_images, output_path):
    """
    Visualize original and reconstructed images side by side using viridis colormap
    
    Args:
        original_images (torch.Tensor): Original images [5, 1, H, W]
        reconstructed_images (torch.Tensor): Reconstructed images [5, 1, H, W] 
        output_path (str): Path to save the visualization
    """
    # Convert to numpy array if needed
    ...
    if isinstance(reconstructed_images, torch.Tensor):
        reconstructed_images = reconstructed_images.squeeze(1).cpu().numpy()
    ...
    for i in range(5):
        ...
        plt.axis('off')
        if i == 2:  # Middle image
            plt.title('Original Images', fontsize=16, fontweight='bold', pad=20)
    
    # Plot reconstructed images in second row
    for i in range(5):
        plt.subplot(2, 5, i + 6)
        plt.imshow(reconstructed_images[i], cmap='viridis')
        plt.axis('off')
        if i == 2:  # Middle image
            plt.title('Reconstructed Images', fontsize=16, fontweight='bold', pad=20)
...
# Create the new reconstruction comparison visualization
visualize_reconstruction_comparison(
    selected_images, samples, 
    os.path.join(output_dir, "reconstruction_comparison_viridis.png")
)
...
# Function to compare distributions with improved styling
def compare_distributions(camels_images, diffusion_images, output_dir):
    ...
    ax[0].plot(bin_mid, train_pdf_mean, color="blue", linewidth=2, label="Original")
    ax[0].plot(bin_mid, test_pdf_mean, color="red", linewidth=2, linestyle="--", label="Model")
    ax[0].set_ylabel(r"$\mu(\rm PDF)$", fontsize=25)
    ax[0].legend(fontsize=22)

    ax[1].plot(bin_mid, train_pdf_std, color="blue", linewidth=2)
    ax[1].plot(bin_mid, test_pdf_std, color="red", linewidth=2, linestyle="--")
    ax[1].set_ylabel(r"$\sigma(\rm PDF)$", fontsize=25)

    for i in range(2):
        ax[i].set_xlabel(r"$N_{\rm HI}$", fontsize=25)
        ax[i].grid(True, alpha=0.7)
        ax[i].tick_params(axis='both', which='major', labelsize=16)
    
    # Add overall title
    fig.suptitle("Probability Distribution", fontsize=28)
    plt.tight_layout()
...
# Plot guidance strength vs. metrics (ELBO, BPD, NLL) with improved styling
plt.figure(figsize=(15, 5))

# Plot guidance strength vs. metrics
plt.subplot(1, 3, 1)
plt.plot([m['guidance'] for m in guided_metrics], [m['elbo'] for m in guided_metrics], 'o-', linewidth=2, markersize=8)
plt.xlabel("Guidance Strength", fontsize=14)
plt.ylabel("ELBO", fontsize=14)
plt.grid(True, alpha=0.7)
plt.title("Guidance Strength vs. ELBO", fontsize=16, fontweight='bold')

plt.subplot(1, 3, 2)
plt.plot([m['guidance'] for m in guided_metrics], [m['bpd'] for m in guided_metrics], 'o-', linewidth=2, markersize=8)
plt.xlabel("Guidance Strength", fontsize=14)
plt.ylabel("Bits Per Dimension (BPD)", fontsize=14)
plt.grid(True, alpha=0.7)
plt.title("Guidance Strength vs. BPD", fontsize=16, fontweight='bold')

plt.subplot(1, 3, 3)
plt.plot([m['guidance'] for m in guided_metrics], [m['nll'] for m in guided_metrics], 'o-', linewidth=2, markersize=8)
plt.xlabel("Guidance Strength", fontsize=14)
plt.ylabel("Negative Log Likelihood (NLL)", fontsize=14)
plt.grid(True, alpha=0.7)
plt.title("Guidance Strength vs. NLL", fontsize=16, fontweight='bold')

plt.tight_layout()
plt.savefig(os.path.join(output_dir, "guidance_metrics.png"), dpi=300, bbox_inches='tight')
plt.close()
...
if num_params > 0:
    ...
    for param_idx in range(num_params):
        ...
        for i, val in enumerate(param_values):
            ...
            axs[param_idx, i].set_title(f"Param {param_idx+1} = {val:.2f}", fontsize=12)
    ...
    # Add overall title
    fig.suptitle("Power Spectrum", fontsize=18, fontweight='bold')
    plt.tight_layout()
    ...
    # Plot parameter value vs. metrics for each parameter with improved styling
    for param_idx in range(num_params):
        ...
        plt.subplot(1, 3, 1)
        plt.plot([m['param_value'] for m in metrics], [m['elbo'] for m in metrics], 'o-', linewidth=2, markersize=8)
        plt.xlabel(f"Parameter {param_idx+1} Value", fontsize=14)
        plt.ylabel("ELBO", fontsize=14)
        plt.grid(True, alpha=0.7)
        plt.title(f"Parameter {param_idx+1} Value vs. ELBO", fontsize=16, fontweight='bold')
        
        plt.subplot(1, 3, 2)
        plt.plot([m['param_value'] for m in metrics], [m['bpd'] for m in metrics], 'o-', linewidth=2, markersize=8)
        plt.xlabel(f"Parameter {param_idx+1} Value", fontsize=14)
        plt.ylabel("Bits Per Dimension (BPD)", fontsize=14)
        plt.grid(True, alpha=0.7)
        plt.title(f"Parameter {param_idx+1} Value vs. BPD", fontsize=16, fontweight='bold')
        
        plt.subplot(1, 3, 3)
        plt.plot([m['param_value'] for m in metrics], [m['nll'] for m in metrics], 'o-', linewidth=2, markersize=8)
        plt.xlabel(f"Parameter {param_idx+1} Value", fontsize=14)
        plt.ylabel("Negative Log Likelihood (NLL)", fontsize=14)
        plt.grid(True, alpha=0.7)
        plt.title(f"Parameter {param_idx+1} Value vs. NLL", fontsize=16, fontweight='bold')
        
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"parameter_{param_idx+1}_metrics.png"), dpi=300, bbox_inches='tight')
        plt.close()
